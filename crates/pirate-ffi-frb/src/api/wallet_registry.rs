...
fn load_wallet_registry(db: &Database) -> Result<(Vec<WalletMeta>, Option<WalletId>)> {
    ...
    let mut stmt = db.conn().prepare(
        "SELECT id, name, created_at, watch_only, birthday_height, network_type, endpoint, overwinter_height, sapling_height, orchard_height
...
pub(super) fn persist_wallet_meta(db: &Database, meta: &WalletMeta) -> Result<()> {
    db.conn().execute(
        ...
        INSERT INTO wallet_registry
            (id, name, created_at, watch_only, birthday_height, network_type, endpoint, overwinter_height, sapling_height, orchard_height, last_used_at, last_synced_at)
